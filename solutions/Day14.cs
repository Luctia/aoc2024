...
public partial class Day14 : Day
{
    public override void Part1()
    {
        ...
        List<(int x, int y)> robotLocations = matchedLines.Select(m => (int.Parse(m[0].Groups[1].Value), int.Parse(m[0].Groups[2].Value))).ToList();
        List<(int x, int y)> robotVelocities = matchedLines.Select(m => (int.Parse(m[0].Groups[3].Value), int.Parse(m[0].Groups[4].Value))).ToList();
    ...
    private static List<(int x, int y)> PerformIterations(List<(int x, int y)> robotLocations, List<(int x, int y)> robotVelocities, (int width, int height) boardSize, int iterations)
    {
        for (int i = 0; i < robotLocations.Count; i++)
        {
            robotLocations[i] = (
                ((robotLocations[i].x + iterations * robotVelocities[i].x) % boardSize.width + boardSize.width) % boardSize.width,
                ((robotLocations[i].y + iterations * robotVelocities[i].y) % boardSize.height + boardSize.height) % boardSize.height);
        }
