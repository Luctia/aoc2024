...
public class Day15 : Day
{
    ...
    private static (Grid, (int, int)) PerformInstructionWideGrid(Grid grid, char instruction, (int x, int y) robotPosition)
    {
        switch (instruction)
        {
            case '>':
            ...
            case 'v':
            {
                ...
                grid = PushWideBoxDown(grid, grid.Under(robotPosition) == '[' ? (robotPosition.x, robotPosition.y + 1) : (robotPosition.x - 1, robotPosition.y + 1));
                if (grid.Under(robotPosition) == '.')
                {
                    grid.SetCharAt(robotPosition, '.');
                    grid.SetCharAt((robotPosition.x, robotPosition.y + 1), '@');
            ...
            case '^':
            {
                ...
                grid = PushWideBoxUp(grid, grid.Above(robotPosition) == '[' ? (robotPosition.x, robotPosition.y - 1) : (robotPosition.x - 1, robotPosition.y - 1));
                if (grid.Above(robotPosition) == '.')
                {
                    grid.SetCharAt(robotPosition, '.');
                    grid.SetCharAt((robotPosition.x, robotPosition.y - 1), '@');
